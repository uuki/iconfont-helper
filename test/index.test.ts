...
it('fontagon style example test case.', async () => {
  ...
  await svgtofont({
    src: path.resolve(process.cwd(), 'test', 'example', 'fonts', 'fontagon.css'),
    dist: dist,
  ...
  expect(fileNames).toEqual(['iconfont-helper.css', 'iconfont-helper.scss']);
  await fs.emptyDir(dist);
});
...
it('svgtofont style example test case.', async () => {
  ...
  await fs.emptyDir(dist);
  await svgtofont({
    src: path.resolve(process.cwd(), 'test', 'example', 'fonts', 'svgtofont.css'),
    dist: dist,
    fontName: 'iconfont', // font name
    fileName: 'iconfont-helper', // file name
